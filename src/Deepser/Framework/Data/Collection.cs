...
namespace Deepser.Framework.Data
{
    public class Collection : Data
    {
        ...
        //GetSingleData method retrieves a single attribute value or entity IDs from a list of JSON objects, and returns the result as a string.
        public object GetSingleData(string attribute)
        {
            ...
            else
            {
                foreach (var item in jsonlist)
                {
                    ...
                    {
                        object id = item.ContainsKey("entity_id") ? item["entity_id"] : item.ContainsKey("user_id") ? item["user_id"] : null;
                        object valueObj = item[attribute];
                        string value = valueObj != null && !string.IsNullOrEmpty(valueObj.ToString()) ? valueObj.ToString() : "Is null or empty";
                        text += id != null ? $"{id}   :   {value}\r\n" : "Key not found\r\n";
                    }
        ...
        public string GetMultipleEndpoint()
        {
            ...
            var _endpoint = $"{(string.IsNullOrEmpty($"{id}") ? $"{this.endpoint}" : $"{activityEndpoint}/{id}{field}{Activity.MULTIPLE_ENDPOINT}")}{filter.filterToString()}{(parameter.parameters.Count > 0 ? "&" + parameter.GetParameter().Substring(1) : parameter.GetParameter())}";
